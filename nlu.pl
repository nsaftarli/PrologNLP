...
preposition(of,X,Y,friend) :- friend(X,Y).
preposition(of,X,Y,friend) :- friend(Y,X).

preposition(of,X,Y,father) :- father(X,Y).
preposition(of,X,Y,mother) :- mother(X,Y).
preposition(of,X,Y,parent) :- parent(X,Y).
preposition(of,X,Y,brother) :- brother(X,Y).
preposition(of,X,Y,sister) :- sister(X,Y).
preposition(of,X,Y,child) :- child(X,Y).
preposition(of,X,Y,relative) :- relative(X,Y).
preposition(of,X,Y,relative) :- relative(Y,X).
preposition(of,X,Y,grandmother) :- grandmother(X,Y).
...
np2([Noun|Rest], Who) :- common_noun(Noun, Who), mods(Rest,Who,Noun).
...
mods(Words, Who, Noun) :-
    ...
    prepPhrase(Start, Who, Noun), mods(End, Who).

prepPhrase([Prep|Rest], Who, Noun) :-
    preposition(Prep, Who, Ref, Noun), np(Rest, Ref).
